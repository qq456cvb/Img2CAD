...
def polar_parameterization(normal_3d, x_axis_3d):
    ...
    gamma = np.arccos(np.clip(np.dot(x_axis_3d, ref_x).round(6), -1, 1))
...
if __name__ == '__main__':
    print(polar_parameterization_inverse(*polar_parameterization(np.array([ 0.1554, -0.,     -0.9879]), np.array([0.9879, 0.,     0.1554]))))
#     a = polar_parameterization(np.array([-0., -0., -1.]), np.array([ 1.,  0., -0.]))
#     print(polar_parameterization_inverse(*[3.142, -2.356,  -0.785]))
#     b = polar_parameterization(np.array([0, 0, -1.]), np.array([0, 1., 0]))
#     print(a, b)