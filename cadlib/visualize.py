...
from cadlib.sketch import Loop, Profile
...
def vec2CADsolid(vec, is_numerical=False, n=256):
    cad = CADSequence.from_vector(vec, is_numerical=is_numerical, n=256)
    # cad = cad.normalize()
    # print("CAD SEQUENCE: ", cad.__str__())
    cad = create_CAD(cad, is_numerical=is_numerical)
    return cad


def create_CAD(cad_seq: CADSequence, is_numerical=False):
    ...
    for extrude_op in cad_seq.seq[1:]:
        new_body = create_by_extrude(extrude_op, is_numerical=is_numerical)
...
def create_by_extrude(extrude_op: Extrude, is_numerical=False):
