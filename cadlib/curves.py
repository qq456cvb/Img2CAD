...
class CurveBase(object):
    ...
    def denumericalize(self, n=256):
        """dequantize curve parameters into floats"""
        raise NotImplementedError
...
class Line(CurveBase):
    ...
    def numericalize(self, n=256):
        self.start_point = ((self.start_point + 1.) / 2. * n).round().clip(min=0, max=n-1).astype(int)
        self.end_point = ((self.end_point + 1.) / 2. * n).round().clip(min=0, max=n-1).astype(int)
        
    def denumericalize(self, n=256):
        self.start_point = self.start_point / n * 2. - 1.
        self.end_point = self.end_point / n * 2. - 1.
    ...
    def draw(self, ax, color):
        ...
        l1 = lines.Line2D(xdata, ydata, lw=10, color=color, axes=ax)     # HACK: Changed lw from 1->10
        ax.add_line(l1)
        ax.plot(self.start_point[0], self.start_point[1], color=color)
...
class Arc(CurveBase):
    ...
    def from_vector(vec, start_point, is_numerical=True):
        end_point = vec[1:3]
        # if is numerical, we directly copy
        sweep_angle = vec[3] if is_numerical else vec[3] / 180 * np.pi
        clock_sign = vec[4]
    ...
    def numericalize(self, n=256):
        self.start_point = ((self.start_point + 1.) / 2. * n).round().clip(min=0, max=n-1).astype(int)
        self.mid_point = ((self.mid_point + 1.) / 2. * n).round().clip(min=0, max=n-1).astype(int)
        self.end_point = ((self.end_point + 1.) / 2. * n).round().clip(min=0, max=n-1).astype(int)
        self.center = ((self.center + 1.) / 2. * n).round().clip(min=0, max=n-1).astype(int)
        tmp = np.array([self.start_angle, self.end_angle])
        # angle can be from -2pi to 2pi
        self.start_angle, self.end_angle = ((tmp + 2 * np.pi) / (4 * np.pi) * n).round().clip(
                                            min=0, max=n-1).astype(int)
        # hack here, start angle is always 0
        self.start_angle = 0
        self.numericalized = True
        
    def denumericalize(self, n=256):
        self.start_point = self.start_point / n * 2. - 1.
        self.mid_point = self.mid_point / n * 2. - 1.
        self.end_point = self.end_point / n * 2. - 1.
        self.center = self.center / n * 2. - 1.
        self.start_angle = self.start_angle / n * 4 * np.pi - 2 * np.pi
        # hack here, start angle is always 0
        self.start_angle = 0.
        self.end_angle = self.end_angle / n * 4 * np.pi - 2 * np.pi
        self.numericalized = False

    def to_vector(self):
        sweep_angle = self.end_angle - self.start_angle
        return np.array([ARC_IDX, self.end_point[0], self.end_point[1], sweep_angle if self.numericalized else sweep_angle / np.pi * 180, int(self.clock_sign), PAD_VAL,
                         *[PAD_VAL] * N_ARGS_EXT])
    ...
    def draw(self, ax, color):
        ...
        ap = patches.Arc(
            ...
            theta1=start_angle,
            theta2=end_angle,
            lw=10,                   # HACK: Changed this from 1 to 10
            color=color
        )
        ax.add_patch(ap)
        ax.plot(self.start_point[0], self.start_point[1], color=color)
        # ax.plot(self.center[0], self.center[1], 'ok', color=color)
        ax.plot(self.mid_point[0], self.mid_point[1], color=color)
        # ax.plot(self.end_point[0], self.end_point[1], 'ok')
...
class Circle(CurveBase):
    ...
    def numericalize(self, n=256):
        self.center = ((self.center + 1.) / 2. * n).round().clip(min=0, max=n-1).astype(int)
        self.radius = ((self.radius + 1.) / 2. * n).round().clip(min=1, max=n-1).astype(int)
    
    # TODO check radius range
    def denumericalize(self, n=256):
        self.center = self.center / n * 2. - 1.
        self.radius = self.radius / n * 2. - 1.
    ...
    def draw(self, ax, color):
        ap = patches.Circle((self.center[0], self.center[1]), self.radius,
                            lw=10, fill=None, color=color)
        ax.add_patch(ap)
        ax.plot(self.center[0], self.center[1])
